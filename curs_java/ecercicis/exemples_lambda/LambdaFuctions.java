...
//1. Recorrer todos los elementos de la colección imprimiendo por pantalla cada uno de ellos
//2. Imprimir todos los alumnos cuyo nombre comience por la letra ‘L’ o ‘G’
//3. Recuperar una nueva colección con alumnos que coinciden con el filtro del enunciado
...
//11. Obtener todos los alumnos cuyo nombre empece por la letra ‘P’ y tenga un longitud menor o
//igual a 6
...
//14. Obtener un mapa que almacene la edad total de los alumnos de cada curso
        ...
        listaAlumnos.stream().
                filter(a -> a.getNombreCurso().equals("PHP") && a.getNota() > 9)
                .forEach(System.out::println);
        ...
        Map<String, Integer> sumEDADPorCurso = listaAlumnos.stream()
                .collect(Collectors.groupingBy(Alumno::getNombreCurso, Collectors.summingInt(Alumno::getEdad)));
        ...
        String str = lstTexto.stream().parallel().collect(StringBuilder::new, (sb, s) -> sb.append(" ").append(s), StringBuilder::append).toString();

        Supplier<StringBuilder> supp = StringBuilder::new;
        BiConsumer<StringBuilder, String> bionsumerAcumulador = (sb, s) -> sb.append(" ").append(s);
        BiConsumer<StringBuilder, StringBuilder> bionsumerCombinador = (sb1, sb2) -> sb1.append("***").append(sb2.toString());
        String strwww = lstTexto.stream().parallel().collect(supp, bionsumerAcumulador, bionsumerCombinador).toString();
